...
class GameSim():
    ...
    def set(self, current_player, black_intersections, white_intersections, groups, board_history, input_history, just_passed, game_over):
    ...
    def step(self, next_move):
        ...
        if next_move not in self.black_intersections[:] + self.white_intersections[:] and self.on_board(next_move):
    ...
    def run(self, displayer, agents):
        while self.game_over == False:
            next_move = self.get_action(displayer, agents[self.current_player])
...
class NNAgent():
    ...
    def ponder(self, gamesim):
        ...
        root = Node(probs)
        while torch.sum(root.visit_counts).item() < 360:
            current_node = root
            ...
            while True:
                ...
                if mcts_gamesim.step(self.intersection_from_move_number(chosen_move)):
                    ...
                    if chosen_move not in current_node.expanded_edges:
                        ...
                        probs = torch.nn.functional.softmax(policy, 0)
                        current_node.expanded_edges[chosen_move] = Node(probs)
        ...
        self.mcts_prob_dist = torch.distributions.Categorical((probs/1600)**(1/1))
        self.network_prob_dist = torch.distributions.Categorical(probs)
